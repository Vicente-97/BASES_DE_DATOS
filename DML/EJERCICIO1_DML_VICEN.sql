...
--No se podria eliminar por que corresponde a una FK de la tabla producto y estamos queriendo modificar el código que se trata de una PK de fabricante.--, la sentencia seria:
...
UPDATE FABRICANTE SET codigo=8 WHERE codigo=30;

--Pues si me deja por que al no estar vinculada de FK en la tabla Producto pues si me deja--
...
ALTER TABLE PRODUCTO DROP CONSTRAINT FK_PRODUCTO;
ALTER TABLE PRODUCTO ADD CONSTRAINT FK_PRODUCTO  FOREIGN KEY (codigo_fabricante) REFERENCES FABRICANTE (codigo) ON DELETE CASCADE;
DELETE FROM FABRICANTE WHERE nombre='Hawlett-Packard';
