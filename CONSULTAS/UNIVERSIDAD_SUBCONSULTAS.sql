...
--Mostrar el nombre de las asignaturas que tienen más créditos que "Seguridad Vial". --
...
--Obtener el Id de los alumnos matriculados en las asignaturas "150212" y "130113" a la vez. --
...
--Mostrar el Id de los alumnos matriculados en las asignatura "150212" ó "130113", en una o en otra pero no en ambas a la vez. 
...
--Mostrar el nombre de las asignaturas de la titulación "130110" cuyos costes básicos sobrepasen el coste básico promedio por asignatura en esa titulación.

SELECT a.NOMBRE 
FROM ASIGNATURA a 
WHERE a.IDTITULACION = '130110' AND a.COSTEBASICO > (SELECT AVG(NVL( a2.COSTEBASICO,0)) FROM ASIGNATURA a2 );
...
--Mostrar el Id de los alumnos matriculados en la asignatura "150212" pero no en la "130113". 
...
--Mostrar el nombre de las asignaturas que tengan más créditos. 
...
FROM PERSONA p 
WHERE p.CIUDAD IN (SELECT DISTINCT  p.CIUDAD  FROM PERSONA p, PROFESOR p2 WHERE p.DNI = p2.DNI) 
AND p.CIUDAD IN (SELECT DISTINCT  p3.CIUDAD  FROM PERSONA p3, ALUMNO a WHERE p3.DNI= a.DNI);
