...
--CREATE USER RESIDUOS_VICENTE IDENTIFIED BY RESIDUOS_VICENTE ;
--GRANT CONNECT, RESOURCE, DBA TO RESIDUOS_VICENTE  ;
...
--Muestra el nombre de las empresas productoras de Huelva o Málaga ordenadas por el
--nombre en orden alfabético inverso.


SELECT e.NOMBRE_EMPRESA 
FROM EMPRESAPRODUCTORA e 
WHERE UPPER(e.CIUDAD_EMPRESA) LIKE 'MALAGA'
OR UPPER(e.CIUDAD_EMPRESA) LIKE 'MÁLAGA'
OR UPPER(e.CIUDAD_EMPRESA) LIKE 'HUELVA'
...
--4 4. Muestra el tipo de transporte, los kilómetros y el coste de los traslados realizados en
--diciembre de 1994.
...
SELECT rc.COD_RESIDUO, COUNT(rc.COD_CONSTITUYENTE) AS NUMERO_CONSTITUYENTE 
FROM RESIDUO_CONSTITUYENTE rc 
...
SELECT NVL(AVG(rc.CANTIDAD),0)
FROM RESIDUO_EMPRESA rc 
WHERE EXTRACT (YEAR FROM rc.FECHA)=1994;

-- Todas las fechas están a 2022 por eso me sale 0--
...
--SELECT t.FECHA_ENVIO, t.KMS  FROM TRASLADO t 


--Me va a mostrar el mayor numero de kilómetros de un traslado en 03 = MARZO, la consulta anterior esta realizada
-- Para ver por consola cual es el mayor numero de kms y comprobar que el ejercicio esta bien--
...
SELECT DISTINCT COUNT(rc.COD_CONSTITUYENTE) AS NUMERO_CONSTITUYENTES_DISTINTOS, re.NIF_EMPRESA 
...
--SELECT e.OD_EMPTRANSPORTE FROM EMPRESATRANSPORTISTA e ;
--SELECT e2.OD_EMPRESA  FROM EMPRESAPRODUCTORA e2;
--SELECT r.OD_RESIDUO  FROM RESIDUO r;
...
SELECT COUNT(d.COD_DESTINO) AS NUMERO_ENVIOS, e.CIUDAD_EMPRESA AS CIUDAD_ORIGEN_PRODUCCION, d.CIUDAD_DESTINO AS CIUDAD_DESTINO_ENVIO
...
FROM EMPRESATRANSPORTISTA e, EMPRESAPRODUCTORA e2, TRASLADO t, RESIDUO r 
WHERE e.NIF_EMPTRANSPORTE = t.NIF_EMPTRANSPORTE 
AND t.COD_RESIDUO = r.COD_RESIDUO 
AND t.NIF_EMPRESA = e2.NIF_EMPRESA 
AND (UPPER(e2.CIUDAD_EMPRESA) LIKE 'MALAGA'
OR  UPPER(e2.CIUDAD_EMPRESA) LIKE 'MÁLAGA'
OR UPPER(e2.CIUDAD_EMPRESA) LIKE 'HUELVA')
AND r.OD_RESIDUO LIKE '%Bario%'
OR r.OD_RESIDUO LIKE '%Lantano%';

--SELECT r.OD_RESIDUO  FROM RESIDUO r;

--Hemos comprobado la descripcion de los distintos residuos y ninguno contiene la palabra Bario o Lantano en su OD.

--12  Mostrar el coste por kilómetro del total de traslados encargados por la empresa productora
--Carbonsur.

SELECT (SUM(t.COSTE)/SUM(t.KMS)) AS PRECIO_POR_KILOMETRO_DE_TRASLADOS_TOTALES
...
SELECT  rc.COD_RESIDUO, COUNT(rc.COD_CONSTITUYENTE) AS NUMERO_DE_CONSTITUYENTES
FROM RESIDUO_CONSTITUYENTE rc
GROUP BY rc.COD_RESIDUO ;
