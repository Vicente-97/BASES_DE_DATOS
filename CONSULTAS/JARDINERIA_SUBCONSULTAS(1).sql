...
--CREATE USER BANCO_CONSULTAS IDENTIFIED BY BANCO_CONSULTAS ;
--GRANT CONNECT, RESOURCE, DBA TO BANCO_CONSULTAS  ;
...
CREATE TABLE TIPO_MOVIMIENTO(
  ...
  SALIDA VARCHAR(5) CHECK(SALIDA='S�' OR SALIDA='No'));
...
FROM CLIENTE c
WHERE c.LIMITE_CREDITO = (SELECT MAX(LIMITE_CREDITO) FROM CLIENTE c2);
...
SELECT p.NOMBRE
FROM PRODUCTO p
WHERE PRECIO_VENTA = (SELECT max(p2.PRECIO_VENTA)
FROM PRODUCTO p2);
