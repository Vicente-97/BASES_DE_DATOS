...
SELECT DISTINCT e.NOMBRE AS empleado, e2.NOMBRE AS jefe 
...
SELECT DISTINCT  c.NOMBRE_CLIENTE 
FROM CLIENTE c, PEDIDO p, PAGO pG
WHERE c.CODIGO_CLIENTE = p.CODIGO_CLIENTE 
...
SELECT count(c.CODIGO_CLIENTE), c.PAIS 
FROM cliente c
GROUP BY c.PAIS
HAVING count(c.CODIGO_CLIENTE) >2;
...
SELECT E.CODIGO_EMPLEADO, COUNT(C.CODIGO_CLIENTE) AS numero_clientes
FROM EMPLEADO E, CLIENTE C
...
SELECT MIN(P.FECHA_PAGO) AS FECHA_MINIMA, MAX(P.FECHA_PAGO) AS FECHA_MAXIMA, P.CODIGO_CLIENTE
FROM PAGO P
GROUP BY P.CODIGO_CLIENTE;

--12 Calcula el número de productos diferentes que hay en cada uno de los pedidos.
SELECT DISTINCT COUNT(DP.CODIGO_PRODUCTO) AS numero_producto ,DP.CODIGO_PEDIDO
FROM DETALLE_PEDIDO DP
GROUP BY DP.CODIGO_PEDIDO;

--13 Calcula la suma de la cantidad total de todos los productos que aparecen en cada uno de los pedidos.
SELECT SUM(DP.CANTIDAD)AS CANTIDAD_TOTAL,DP.CODIGO_PEDIDO
FROM DETALLE_PEDIDO DP
GROUP BY DP.CODIGO_PEDIDO;
...
sum(DP.PRECIO_UNIDAD *DP.CANTIDAD)*0.21 AS PRECIO_IVA,
sum(DP.PRECIO_UNIDAD *DP.CANTIDAD) + sum(DP.PRECIO_UNIDAD *DP.CANTIDAD)*0.21AS PRECIO_TOTAL
FROM DETALLE_PEDIDO DP;



--16 La misma información que en la pregunta anterior, pero agrupada por código de producto.
SELECT DP.CODIGO_PRODUCTO,  sum(DP.PRECIO_UNIDAD *DP.CANTIDAD) AS BASE_IMPONIBLE,
sum(DP.PRECIO_UNIDAD *DP.CANTIDAD)*0.21 AS PRECIO_IVA,
sum(DP.PRECIO_UNIDAD *DP.CANTIDAD) + sum(DP.PRECIO_UNIDAD *DP.CANTIDAD)*0.21 AS PRECIO_TOTAL
FROM DETALLE_PEDIDO DP
GROUP BY DP.CODIGO_PRODUCTO ;
...
SELECT P.NOMBRE,SUM(DP.CANTIDAD) AS UNIDADES_VENDIDAS, SUM((DP.CANTIDAD*PRECIO_UNIDAD)) AS TOTAL_FACTURADO,
...
HAVING SUM((DP.CANTIDAD*PRECIO_UNIDAD));
...
--  2 Devuelve el nombre, apellido1 y cargo de los empleados que no representen a ningÃºn cliente.--
...
--3 Devuelve el nombre del producto que tenga el precio de venta mÃ¡s caro.
SELECT * FROM PRODUCTO;
SELECT NOMBRE FROM PRODUCTO WHERE PRECIO_VENTA = (SELECT MAX(PRECIO_VENTA) FROM PRODUCTO);

--4 Devuelve el nombre del producto del que se han vendido mÃ¡s unidades. (Ten en cuenta que tendrÃ¡s que calcular cuÃ¡l es el nÃºmero total de unidades que se han vendido de cada producto a partir de los datos de la tabla detalle_pedido. Una vez que sepas cuÃ¡l es el cÃ³digo del producto, puedes obtener su nombre fÃ¡cilmente.)
SELECT p.NOMBRE
...
--5 Los clientes cuyo lÃ­mite de crÃ©dito sea mayor que los pagos que haya realizado.
...
--6 El producto que mÃ¡s unidades tiene en stock y el que menos unidades tiene.
SELECT NOMBRE
...
SELECT c.NOMBRE_CLIENTE , SUM(NVL(p.TOTAL,0)) AS DINERO_TOTAL_PAGADO
FROM CLIENTE c , PAGO p 
WHERE c.CODIGO_CLIENTE = p.CODIGO_CLIENTE(+)
GROUP BY c.NOMBRE_CLIENTE;
...
SELECT c.NOMBRE_CLIENTE, e.NOMBRE ||' '|| e.APELLIDO1, UPPER(o.CIUDAD)
FROM CLIENTE c, EMPLEADO e, OFICINA o 
WHERE c.CODIGO_EMPLEADO_REP_VENTAS =e.CODIGO_EMPLEADO 
AND e.CODIGO_OFICINA = o.CODIGO_OFICINA;
